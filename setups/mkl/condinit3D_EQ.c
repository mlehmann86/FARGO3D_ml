...
masterprint("INITIALIZING DUST LAYER IN VERTICAL EQUILIBRIUM\n");
...
//      introduce settled dust layer in vertical equilibrium
	//alpha=NUD/pow(ASPECTRATIO,2.0);
	//delta=(1.+STOKES1+4.*pow(STOKES1,2.0))/pow(1.+pow(STOKES1,2.0),2.0)*alpha;
	delta=NUD/(cs*H);


//-------------------------------------------------------------------
	//Dullemond et al pressure bump equilibrium solution (Eq. F12)
//	if(bump_amp > 0 && FLARINGINDEX == 0.5){

//	wd=BUMP_WIDTH*sqrt(delta*(STOKES1 +pow(STOKES1,-1.0)));

//	dgratio*=(1.0 + 2.0*exp(-0.5*pow((R-R0)/(wd*H0),2.0)));
//	}
//-------------------------------------------------------------------
	...
	//delta=alpha;
	bet=1./2./St*(1.-sqrt(1.-4.*pow(St,2.0)));
	//bet=STOKES1;
	dgratio *=exp(-bet/2./delta*pow(z/H,2.0));


	//printf("%2.6f", exp(-bet/2./delta*pow(z/H,2.0)));

     
      vert = delta/St*(dgratio-EPSILON)-0.5*pow(z/H,2.0); 
      rhog*= exp(vert); 
      rhog_c = rhog;
...
//-------------------------------------------------------------------
	//Dullemond et al equilibrium solution (Eq. F12)
//	if(bump_amp > 0 && FLARINGINDEX == 0.5){

//	vrad_dust=-cs2/(pow(BUMP_WIDTH,2.0)*omega_kep*(STOKES1+pow(STOKES1,-1.0)))*(R-R0);
//	}
//-------------------------------------------------------------------

          vtheta_gas=0;// vR_gas*cos(Zmin(k));
...
//          rho[l] += 0.01*rdnum*rho[l] ;    //*****MODIFICATION*************
 ...
 //r = Ymed(j);
	...
	if ((j >= 0) && (j < Ny+2*NGHY) && (k >= 0) && (k < Nz+2*NGHZ)  && (rho[l]+rho_tmp > 1.0e-7) ) {
	  rho[l] += rho_tmp;
	}
