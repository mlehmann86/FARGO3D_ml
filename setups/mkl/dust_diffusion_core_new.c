...
//<#>
	...
	update = 0.0;
	c    = rhod[ll]/rhog[ll]; //Cell centered
	
	// DUST DIFFUSION ALONG X-DIRECTION
	
#ifdef X
        d1   = 0.25*(rhog[ll] + rhog[llxp])*(sdiff_yczc[llxp]+sdiff_yczc[ll]); //face centered in X
        d2   = 0.25*(rhog[ll] + rhog[llxm])*(sdiff_yczc[llxm]+sdiff_yczc[ll]); //face centered in X
	cxp  = rhod[llxp]/rhog[llxp];                                                   //Cell centered
        cxm  = rhod[llxm]/rhog[llxm];                                                   //Cell centered
...
#ifdef Y
	d3   = 0.5*(rhog[ll] + rhog[llyp])*sdiff_yfzc[llyp];//face centered in Y
	d4   = 0.5*(rhog[ll] + rhog[llym])*sdiff_yfzc[ll];  //face centered in Y
	cyp  = rhod[llyp]/rhog[llyp];                                //Cell centered
	cym  = rhod[llym]/rhog[llym];                                //Cell centered
...
#ifdef Z
	d5   = 0.5*(rhog[ll] + rhog[llzp])*sdiff_yczf[llzp]; // face centered in Z
	d6   = 0.5*(rhog[ll] + rhog[llzm])*sdiff_yczf[ll];   // face centered in Z
	czp  = rhod[llzp]/rhog[llzp];                                 // Cell centered
	czm  = rhod[llzm]/rhog[llzm];                                 // Cell centered
#ifdef CARTESIAN
