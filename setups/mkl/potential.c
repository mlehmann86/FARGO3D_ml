...
#ifdef UNSTRAT
        pot[l] =  -G*MSTAR/ymed(j); //unstratified potential from star
#endif


#ifdef DUSTVG //unstratified only for gas

	if(fluidtype==GAS) {
//printf("COMPUTE POTENTIAL: GAS = %f\n",fluidtype);
        pot[l] =  -G*MSTAR/sqrt(ymed(j)*ymed(j)); 
}
	if(fluidtype==DUST) {
//printf("COMPUTE POTENTIAL: DUST = %f\n",fluidtype);
//printf("Phyical Time= %f\n",PhysicalTime);

        pot[l] =  -G*MSTAR/sqrt(ymed(j)*ymed(j)+ZC*ZC);
//	pot[l]*= (1.0 - exp(-PhysicalTime/0.5));  //switch on smoothly
...
tauc=1.0 + (1.0-taucbcgas)*tanh(flan*(bigrad-TaperRin));

tauc=tauc - (1.0-taucbcgas)*tanh(flan*(bigrad-TaperRout));

tauc=0.5*(tauc-1.0) +taucbcgas;


pot[l] =  -G*MSTAR/sqrt(ymed(j)*ymed(j)+ZC*ZC*tauc);
	...
	if (indirect_term == YES) {
		pot[l] -= indirectx*XC + indirecty*YC; /* Indirect term due to gas */
	}
#else
	if (indirect_term == YES) {
	...
	for(n=0; n<nb; n++) {
	  mp = mplanet[n]*taper*potfac; //introduced potfac 05.03.2025
	  ...
	  dist = ((XC-xplanet[n])*(XC-xplanet[n])+
       ...
       dist_xy = ((XC-xplanet[n])*(XC-xplanet[n]) +
                 (YC-yplanet[n])*(YC-yplanet[n]));
#else
	  dist = ((XC-xplanet[n])*(XC-xplanet[n])+
		  (YC-yplanet[n])*(YC-yplanet[n])+
...
#ifdef UNSTRAT
          //pot[l] += -G * mp / (ZMAX - ZMIN) * log(
          //           sqrt(dist_xy + ZMAX * ZMAX + smoothing) /
          //           sqrt(dist_xy + ZMIN * ZMIN + smoothing)); //wire

	  pot[l] += -G*mp/sqrt(dist_xy+smoothing); //unstrat
#else
