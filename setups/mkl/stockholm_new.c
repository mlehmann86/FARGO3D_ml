...
//<#>
	...
	if(ramp>0.0) {
      ...
      if (fltype == DUST) { 
	if(ymed(j) > Y_sup) {  //!!!!!!!!!!!!!!!!!! DUST DENSITY ONLY AT OUTER RADIAL BOUNDARY !!!!!!!!!!!!!!!!!!!//
...
// EDIT MLEHMANN 16.05.2022
...
#ifdef Y
    //  if (fltype == DUST) { 
	//if(ymed(j) > Y_sup) {  //!!!!!!!!!!!!!!!!!! RADIAL (DUST) VELOCITY ONLY AT OUTER RADIAL BOUNDARY !!!!!!!!!!!!!!!!!!!//
	...
	//  vy[l] = (vy[l]*taud+vy0[l2D]*dt)/(dt+taud);  
	//}
	//  vy[l] = (vy[l]*taud)/(dt+taud); // damp radial velocity to 0 at boundaries?
#endif
	}
#ifdef Z
	if(rampi>0.0) {
