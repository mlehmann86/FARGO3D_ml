...
//<#>
	...
	if(ramp>0.0) {
      ...
      if (fltype == DUST) { 
	//if(ymed(j) > Y_sup) {  //!!!!!!!!!!!!!!!!!! ONLY AT OUTER RADIAL BOUNDARY !!!!!!!!!!!!!!!!!!!//
...
//	}
       ...
       if (fltype == GAS) { // EDIT MLEHMANN 12.12.2023
// EDIT MLEHMANN 16.05.2022
#ifdef ADIABATIC
	//if(ymed(j) < Y_inf) { //!!!!!!!!!!!!!!!!!! ONLY AT INNER RADIAL BOUNDARY !!!!!!!!!!!!!!!!!!!//
 ...
 //     if (fltype == DUST) { 
//	  vx[l] = (vx[l]*taud+vx0_target*dt)/(dt+taud);//!!!!!!!!!!!!!!!!!! ONLY AT OUTER RADIAL BOUNDARY !!!!!!!!!!!!!!!!!!!//
//	}
  //    if (fltype == GAS) { 
//	if(ymed(j) > Y_sup) { 
	  vx[l] = (vx[l]*taud+vx0_target*dt)/(dt+taud);//!!!!!!!!!!!!!!!!!! ONLY AT OUTER RADIAL BOUNDARY !!!!!!!!!!!!!!!!!!!//
...
#ifdef Y
 //     if (fltype == DUST) { 
//	  vy[l] = (vy[l]*taud+vy0[l2D]*dt)/(dt+taud);  //!!!!!!!!!!!!!!!!!! ONLY AT OUTER RADIAL BOUNDARY !!!!!!!!!!!!!!!!!!!//
//		}
 //     if (fltype == GAS) { 
	// if(ymed(j) > Y_sup) { 
	  vy[l] = (vy[l]*taud+vy0[l2D]*dt)/(dt+taud);  //!!!!!!!!!!!!!!!!!! ONLY AT OUTER RADIAL BOUNDARY !!!!!!!!!!!!!!!!!!!//
...
//	}
	...
	if(rampi>0.0) {
	  taud = tau/rampi;
  //    if (fltype == DUST) { 
//	  vz[l] = (vz[l]*taud+vz0[l2D]*dt)/(dt+taud);//!!!!!!!!!!!!!!!!!! ONLY AT OUTER RADIAL BOUNDARY !!!!!!!!!!!!!!!!!!!//
//	}
//      if (fltype == GAS) { 
	//if(ymed(j) > Y_sup) { 
	  vz[l] = (vz[l]*taud+vz0[l2D]*dt)/(dt+taud);//!!!!!!!!!!!!!!!!!! ONLY AT OUTER RADIAL BOUNDARY !!!!!!!!!!!!!!!!!!!//
